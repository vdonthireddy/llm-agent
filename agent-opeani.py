...
# llm = OpenAI()#ChatOllama(model="llama2:latest")

# setup the simple tools using LangChain tool decorator
@tool
def add(a: int, b: int) -> int:
    """Add two numbers."""
    print('calling add vj')
    return a + b

@tool
def multiply(a: int, b: int) -> int:
    """Multiply two numbers."""
    print('calling multiply vj')
    return a * b
...
def execute_agent(state):
    ...
    print('calling execute_agent vj')
...
def execute_tools(state):
    print('calling execute_tools vj')
...
def should_continue(data):
    print('calling should_continue vj')
...
inputs = {"input": "what is 1 plus 1 and tell me if it is a iseB or iraS sankya, and what is 13 times 7 and tell me if it is a iseB or iraS sankya and finally how is the weather in San Ramon?", "chat_history": []} 
config = {"configurable": {"thread_id": "1"}}

print('calling initial vj')
